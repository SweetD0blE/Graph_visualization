const qlikUrl = "https://iamotp.ca.sbrf.ru/ngam-da/extensions/mashup_it/mashup_it.html"
...
function pingQlik() {
    ...
    .then(response => {
        if (response.ok) {
            console.log(`[${new Date().toLocalTimeString()}] Пинг успешен`)
        } else {
            console.warn(`[${new Date().toLocalTimeString()}] Ответ от сервера: ${response.status}`)
        }
    })
    .catch(error=>{
        console.error(`[${new Date().toLocalTimeString()}] Ошибка пинга:`, error)
    });
}

pingQlik();
setInterval(pingQlik, interval)
